...
CREATE TABLE IF NOT EXISTS `messages` (
    ...
    `content`     TEXT NOT NULL,
    `message_type` ENUM('text', 'meetup') DEFAULT 'text' COMMENT 'Loại tin nhắn',
    `meetup_location` VARCHAR(255) DEFAULT NULL COMMENT 'Địa điểm hẹn gặp (nếu message_type=meetup)',
    `meetup_time` DATETIME DEFAULT NULL COMMENT 'Thời gian hẹn gặp',
    `meetup_status` ENUM('pending', 'accepted', 'rejected', 'cancelled') DEFAULT NULL COMMENT 'Trạng thái lịch hẹn',
